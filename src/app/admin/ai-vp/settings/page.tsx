...
import { Settings, Save, RefreshCw, History, AlertTriangle, Info } from 'lucide-react';
...
export default function AiVpSettingsPage() {
  ...
  const fetchSettings = async () => {
    try {
      const res = await fetch('/api/admin/ai-vp-settings?includeAudit=true');
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/admin/ai-vp-settings', {
        method: 'POST',
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-medium">
                            {entry.action === 'reset' ? 'リセット' : entry.action === 'create' ? '作成' : '更新'}
                          </span>
                          <span className="text-gray-500 text-xs">
                            {new Date(entry.timestamp).toLocaleString('ja-JP')}
                          </span>
                        </div>
                        <p className="text-gray-500 text-xs mb-1">by {entry.userId}</p>
                        {entry.changes.length > 0 && (
                          <ul className="text-xs text-gray-600 list-disc list-inside">
                            ...
                            {entry.changes.length > 5 && (
                              <li>...他 {entry.changes.length - 5} 件</li>
                            )}
                          </ul>
                        )}
                      </div>
                  ...
                  <>
                    <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="w-20">
                        <Input
                          type="number"
                          value={config.diversity.top3Limit}
                    ...
                    <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="w-20">
                        <Input
                          type="number"
                          value={config.diversity.globalTopLimit}
