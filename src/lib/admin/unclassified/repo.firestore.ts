/**
 * Unclassified Management リポジトリ（Firestore版）
 *
...
const UNCLASSIFIED_EVENTS_COLLECTION = 'unclassified_assign_events';
...
export async function listUnclassifiedEvents(limit: number = 50): Promise<ScopeBackfillEvent[]> {
  const db = getAdminDb();
  const snapshot = await db
    .collection(UNCLASSIFIED_EVENTS_COLLECTION)
...
async function recordUnclassifiedAssignEvent(
  ...
  const docRef = db.collection(UNCLASSIFIED_EVENTS_COLLECTION).doc();
  const eventData = {
...
export async function assignBusinessUnit(
...
): Promise<{ success: true; data: UnclassifiedAssignResponse } | { success: false; error: string }> {
  ...
  // 監査ログ記録（Firestore）
  const event = await recordUnclassifiedAssignEvent(
    viewer,
    entityType,
    ids,
    targetBusinessUnitId,
